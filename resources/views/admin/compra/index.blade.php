...
@section('title', 'ADMIN-COMPRAS')

@section('content')
    <div class="card mt-4">
        <div class="card-body text-center">
            <h1>ADMINISTRACIÓN DE LAS COMPRAS</h1>
        </div>
    </div>
    <div class="card mt-4">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-12">
                    <div class="mb-2">
                        ...
                        <a href="{{ route('admin.compra.show') }}" class="btn btn-primary btn-sm">
                            <i class="fas fa-list"></i> {{ __('Listar compras') }}
                        </a>
                        @endcan
                        @can('admin.proveedores.index')
                        <a href="{{ route('admin.proveedores.index') }}" class="btn btn-primary btn-sm">
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-2">
                                    <div class="input-group">
                                        ...
                                        <input id="buscarProveedor" class="form-control" type="text" placeholder="Nombre del cliente" value="{{ $proveedor->nombre ?? '' }}">
                                ...
                                <div class="col-md-4 mb-2">
                                    <div class="input-group">
                                        ...
                                        <input id="dir_proveedor" class="form-control" type="text"
...
@section('js')
    ...
    <script>
        var compraUrl = "{{ route('admin.compra.index') }}";
        var ticketUrl = "{{ route('admin.compra.ticket', ['id' => ':compra']) }}";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            $("#buscarProveedor").autocomplete({
                source: function(request, response) {
                    $.ajax({
                        url: "{{ route('admin.compra.proveedor') }}",
                        dataType: "json",
                        data: {
                            term: request.term
                        },
                            ...
                            fetch(compraUrl, {
                                ...
                                .then(data => {
                                    ...
                                    if (data.icon === 'success') {
                                        setTimeout(() => {
                                            window.open(
                                                `${ticketUrl.replace(':compra', data.ticket)}`,
                                                '_blank');
